              ...
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex items-center gap-1 flex-shrink-0">
                  {!isSp && ann.status === 'Draft' && canCreate && (
                    <>
                      <button aria-label="Edit announcement" onClick={e => { e.stopPropagation(); openEditModal(ann) }} className="action-btn-blue"><Edit size={14} /> Edit</button>
                      <button aria-label="Send announcement" onClick={e => { e.stopPropagation(); setSendTarget(ann) }} className="action-btn-green"><Send size={14} /> Send</button>
                    </>
                  )}
                  {/* FIX: C-2 — route through confirmation state; FIX: H-4 — aria-label */}
                  {!isSp && canCreate && <button aria-label="Delete announcement" onClick={e => { e.stopPropagation(); setAnnToDelete(ann) }} className="action-btn-red"><Trash2 size={14} /> Delete</button>}
                </div>
              </div>
            </div>
          ))
