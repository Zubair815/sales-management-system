                  ...
                  <tr key={p.id}>
                    ...
                    <td data-label="Actions" data-cell="actions">
                      <div className="flex flex-wrap gap-1 justify-end md:justify-start">
                        <button onClick={() => setViewItem(p)} className="action-btn-blue" aria-label="View payment"><Eye size={14} /> View</button>
                        <button onClick={() => openPrint(p)} className="action-btn-gray" aria-label="Print receipt"><Printer size={14} /> Print</button>
                        {canVerify && p.status === 'Pending' && (
                          <>
                            <button onClick={() => verify(p.id)} disabled={actionLoadingId === `verify_${p.id}`} className="action-btn-green" aria-label="Verify payment">{actionLoadingId === `verify_${p.id}` ? <ButtonSpinner size={14} /> : <Check size={14} />} Verify</button>
                            <button onClick={() => { setRejectTarget(p); rst2() }} className="action-btn-red" aria-label="Reject payment"><X size={14} /> Reject</button>
                          </>
                        )}
                      </div>
                    </td>
