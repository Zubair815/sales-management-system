                  ...
                  <tr key={o.id}>
                    ...
                    <td data-label="Actions" data-cell="actions">
                      <div className="flex flex-wrap items-center justify-end gap-2 md:justify-start">
                        <button onClick={() => setViewOrder(o)} className="flex items-center gap-1 px-2 py-1 bg-blue-50 text-blue-600 hover:bg-blue-100 rounded text-xs font-medium transition-colors" aria-label="View order"><Eye size={14} /> View</button>
                        <button onClick={() => openPrint(o.id)} className="flex items-center gap-1 px-2 py-1 bg-gray-50 text-gray-700 hover:bg-gray-200 rounded text-xs font-medium transition-colors" aria-label="Print order"><Printer size={14} /> Print</button>
                        
                        {/* New Submit Button for Salesperson */}
                        {isSp && o.status === 'Prepared' && <button onClick={() => handleSubmitOrder(o.id)} disabled={actionLoadingId === `submit_${o.id}`} className="flex items-center gap-1 px-2 py-1 bg-green-50 text-green-600 hover:bg-green-100 rounded text-xs font-medium transition-colors disabled:opacity-50" aria-label="Submit order to admin">{actionLoadingId === `submit_${o.id}` ? <ButtonSpinner size={14} /> : <Send size={14} />} Submit</button>}

                        {canApprove && o.status === 'Pending' && <button onClick={() => statusAction(o.id, 'approve')} disabled={actionLoadingId === `approve_${o.id}`} className="flex items-center gap-1 px-2 py-1 bg-green-50 text-green-600 hover:bg-green-100 rounded text-xs font-medium transition-colors disabled:opacity-50" aria-label="Approve order">{actionLoadingId === `approve_${o.id}` ? <ButtonSpinner size={14} /> : <Check size={14} />} Approve</button>}
                        {canApprove && o.status === 'Approved' && <button onClick={() => statusAction(o.id, 'dispatch')} disabled={actionLoadingId === `dispatch_${o.id}`} className="flex items-center gap-1 px-2 py-1 bg-indigo-50 text-indigo-600 hover:bg-indigo-100 rounded text-xs font-medium transition-colors disabled:opacity-50" aria-label="Dispatch order">{actionLoadingId === `dispatch_${o.id}` ? <ButtonSpinner size={14} /> : <Truck size={14} />} Dispatch</button>}
                        {canApprove && o.status === 'Dispatched' && <button onClick={() => statusAction(o.id, 'deliver')} disabled={actionLoadingId === `deliver_${o.id}`} className="flex items-center gap-1 px-2 py-1 bg-purple-50 text-purple-600 hover:bg-purple-100 rounded text-xs font-medium transition-colors disabled:opacity-50" aria-label="Mark order delivered">{actionLoadingId === `deliver_${o.id}` ? <ButtonSpinner size={14} /> : <CheckCircle size={14} />} Deliver</button>}
                        {(o.status === 'Pending' || o.status === 'Prepared' || (canApprove && o.status === 'Approved')) && <button onClick={() => setCancelTarget(o)} className="flex items-center gap-1 px-2 py-1 bg-red-50 text-red-600 hover:bg-red-100 rounded text-xs font-medium transition-colors" aria-label="Cancel order"><XCircle size={14} /> Cancel</button>}
                      </div>
                    </td>
                  </tr>
                ))}
