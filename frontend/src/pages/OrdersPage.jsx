                  ...
                  <tr key={o.id}>
                    ...
                    <td data-label="Actions" data-cell="actions">
                      <div className="flex flex-wrap items-center justify-end gap-1 md:justify-start">
                        <button onClick={() => setViewOrder(o)} className="action-btn-blue" aria-label="View order"><Eye size={14} /> View</button>
                        <button onClick={() => openPrint(o.id)} className="action-btn-gray" aria-label="Print order"><Printer size={14} /> Print</button>
                        
                        {/* New Submit Button for Salesperson */}
                        {isSp && o.status === 'Prepared' && <button onClick={() => handleSubmitOrder(o.id)} disabled={actionLoadingId === `submit_${o.id}`} className="action-btn-green" aria-label="Submit order to admin">{actionLoadingId === `submit_${o.id}` ? <ButtonSpinner size={14} /> : <Send size={14} />} Submit</button>}

                        {canApprove && o.status === 'Pending' && <button onClick={() => statusAction(o.id, 'approve')} disabled={actionLoadingId === `approve_${o.id}`} className="action-btn-green" aria-label="Approve order">{actionLoadingId === `approve_${o.id}` ? <ButtonSpinner size={14} /> : <Check size={14} />} Approve</button>}
                        {canApprove && o.status === 'Approved' && <button onClick={() => statusAction(o.id, 'dispatch')} disabled={actionLoadingId === `dispatch_${o.id}`} className="action-btn-indigo" aria-label="Dispatch order">{actionLoadingId === `dispatch_${o.id}` ? <ButtonSpinner size={14} /> : <Truck size={14} />} Dispatch</button>}
                        {canApprove && o.status === 'Dispatched' && <button onClick={() => statusAction(o.id, 'deliver')} disabled={actionLoadingId === `deliver_${o.id}`} className="action-btn-purple" aria-label="Mark order delivered">{actionLoadingId === `deliver_${o.id}` ? <ButtonSpinner size={14} /> : <CheckCircle size={14} />} Deliver</button>}
                        {(o.status === 'Pending' || o.status === 'Prepared' || (canApprove && o.status === 'Approved')) && <button onClick={() => setCancelTarget(o)} className="action-btn-red" aria-label="Cancel order"><XCircle size={14} /> Cancel</button>}
                      </div>
                    </td>
                  </tr>
                ))}
