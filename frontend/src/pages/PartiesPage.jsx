...
export function PartiesPage() {
  ...
  const toggleStatus = async (item) => {
    try { await api.patch(`/parties/${item.id}/status`); fetch() } catch { toast.error('Failed') }
  }

  const deleteItem = async () => {
    try { await api.delete(`/parties/${deleteTarget.id}`); setDeleteTarget(null); fetch() } catch { toast.error('Failed') }
  }
