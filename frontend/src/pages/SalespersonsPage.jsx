    ...
    <tr key={sp.id}>
      ...
      <td data-label="Actions" data-cell="actions">
        <div className="flex flex-wrap items-center gap-1 justify-end md:justify-start">
          ...
          {canEdit && <button aria-label="Edit salesperson" onClick={() => openEdit(sp)} className="action-btn-blue"><Edit size={14} /> Edit</button>}
          {canCreate && <button aria-label="Toggle salesperson status" onClick={() => toggleStatus(sp)} disabled={actionLoadingId === `toggle_${sp.id}`} className="action-btn-yellow">{actionLoadingId === `toggle_${sp.id}` ? <ButtonSpinner size={14} /> : sp.status === 'Active' ? <ToggleRight size={14} /> : <ToggleLeft size={14} />} {sp.status === 'Active' ? 'Deactivate' : 'Activate'}</button>}
          {canCreate && <button aria-label="Reset password" onClick={() => { setResetTarget(sp); rst2() }} className="action-btn-green"><Key size={14} /> Reset Pwd</button>}
          {canCreate && <button aria-label="Delete salesperson" onClick={() => setDeleteTarget(sp)} className="action-btn-red"><Trash2 size={14} /> Delete</button>}
        </div>
      </td>
    </tr>
